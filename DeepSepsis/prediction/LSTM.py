...
class lstm():
    ...
    def RNN(self, x, m, delta, mean, x_lengths):
        with tf.variable_scope('LSTMSimple', reuse=tf.AUTO_REUSE):
            ...
            # Uncomment the following three lines for missingness and time
            # delay information in model
            ...
            # Uncomment the following three lines for missingness information in model
            ...
            lstm_simple_cell = LSTMCell(num_units=self.n_hidden_units,
                                        activation=None,
                                        reuse=tf.AUTO_REUSE)

            lstm_simple_cell = tf.nn.rnn_cell.DropoutWrapper(lstm_simple_cell,
                                                      output_keep_prob=self.keep_prob)

            init_state = lstm_simple_cell.zero_state(self.batch_size, dtype=tf.float32)
    ...
    def build(self):
        ...
        self.class_ratio = (1.0 * negative_class) / ((
                    0.05 * negative_class))  # - Change class ratio - left for experimentation
        ...
        self.utility = (self.y * self.y_act) * self.utp + (
                    self.y * (1.0 - self.y_act)) * self.ufn + (
                                   (1.0 - self.y) * self.y_act) * self.ufp
        ...
        self.metric_op = tf.group(self.acc_update, self.accuracy,
                                  ...
                                  self.fp, self.tpr, self.fpr, self.class_ratio)
    ...
    def train(self):
        ...
        while epochcount < self.epochs:
            ...
            for train_x, train_y, train_m, train_delta, train_xlen, y_mask, utp, ufp, ufn in dataset.getNextBatch():
                _, loss, summary_str, acc, _, cr = self.sess.run(
                    [self.train_op, self.loss, self.sum, self.accuracy,
                     self.metric_op, self.class_ratio], feed_dict={
                        self.x: train_x,
                        self.y: train_y,
                        self.m: train_m,
            ...
            acc, auc = self.test()
            print(
                "epoch is : %2.2f, Accuracy: %.8f, AUC: %.8f, Class Ratio: %.8f" % (
                epochcount, acc, auc, cr))
