...
from tensorflow.contrib.rnn import LayerRNNCell, LSTMCell
...
class LSTMSimpleCell(LayerRNNCell):
  """
  ...
  # def call(self, inputs, prev_ht, prev_ct):
  def call(self, inputs, state):
    """LSTM Simple Cell"""
    inputs = inputs[:, 0:self._num_units] #<---- IS THIS LINE NEEDED?
    prev_ht = state[0]
    prev_ct = state[1]
    print(inputs.shape) # why is shape (?, #)
    print(state.shape)
    print(prev_ht.shape)
    print(prev_ct.shape)
    concat_input = tf.concat([inputs, prev_ht],axis=0)
    ft = math_ops.sigmoid(concat_input)
    it = math_ops.sigmoid(concat_input)
    candidate = math_ops.tanh(concat_input)
    ct = prev_ct*ft + candidate*it
    ot = math_ops.sigmoid(concat_input)
    ht = ot*math_ops.tanh(ct)

    new_h_drop = tf.nn.dropout(ht, keep_prob=self._dropout_rate)
  ...
  def get_config(self):
    ...
    base_config = super(LSTMSimpleCell, self).get_config()
