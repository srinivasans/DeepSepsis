...
import argparse
import numpy as np
from Physionet2019SepsisData import read_sepsis_data, read_sepsis_test_data
import os

"""main"""
def main():
    cur_dir = os.getcwd()
    data_path = cur_dir + "/../../sepsis_data_txt/"
    ...
    parser.add_argument('--n-inputs', type=int, default=35)
    ...
    for beta1 in beta1s:
        for e in epochs:
            for g_l in g_loss_lambdas:
                ...
                dt_train=read_sepsis_data.ReadPhysionetSepsisData(os.path.join(args.data_path,"train"), os.path.join(args.data_path,"train","list.txt"),isNormal=args.isNormal,isSlicing=args.isSlicing)
                # dt_train=read_sepsis_data.ReadPhysionetSepsisData(args.data_path, os.path.join(args.data_path,"list.txt"),isNormal=args.isNormal,isSlicing=args.isSlicing)
                dt_test=read_sepsis_test_data.ReadPhysionetSepsisData(os.path.join(args.data_path,"test"), os.path.join(args.data_path,"test","list.txt"),dt_train.maxLength, isNormal=args.isNormal,isSlicing=args.isSlicing)
                tf.reset_default_graph()
