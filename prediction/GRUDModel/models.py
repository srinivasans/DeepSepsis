...
from keras.layers import Activation, Dense, Dropout, Input, Masking, Reshape, Lambda, TimeDistributed
...
from .grud_layers import Bidirectional_for_GRUD, GRUD
from .layers import ExternalMasking
...
def create_grud_model(input_dim, recurrent_dim, hidden_dim,
    ...
    grud_layer = GRUD(units=recurrent_dim[0],
                      return_sequences=True, # return_sequences=len(recurrent_dim) > 1,
                      activation='sigmoid',
                      dropout=0.3,
    ...
    for hd in hidden_dim:        
        ...
        x = Activation('relu')(x)
    x = TimeDistributed(Dense(output_dim, activation=output_activation))(x)
    #print(type(x))
    #x = tf.reshape(x, [-1,336,output_dim])
    #x = Lambda(lambda x: x, output_shape=lambda s:s)(x)
    #x = Reshape((-1,))(x)
    output_list = [x]
