"""
Impute sepsis data using GRU-I
@author: Srinivasan Sivanandan
Adapted from GRUI implementation by @author: yonghong, luo
"""
...
import argparse
import numpy as np
from datautils import read_train_data, read_test_data
import os
    ...
    for beta1 in beta1s:
        for e in epochs:
            for g_l in g_loss_lambdas:
                ...
                dt_train=read_train_data.ReadSepsisData(os.path.join(args.data_path,"train"), isNormal=args.isNormal)
                dt_test=read_test_data.ReadSepsisData(os.path.join(args.data_path,"test"), dt_train.maxLength,isNormal=args.isNormal)
                tf.reset_default_graph()
