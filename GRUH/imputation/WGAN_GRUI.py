...
class WGAN(object):
    ...
    def build_model(self):
        ...
        # Mean Squared loss for pretrain autoencoder for generator
    ...
    def imputation(self,dataset,isTrain):
        ...
        impute_tune_time=1
        counter=1
        for data_x,data_y,data_mean,data_m,data_deltaPre,data_x_lengths,data_lastvalues,filenames,imputed_deltapre,imputed_m,deltaSub,subvalues,imputed_deltasub in self.datasets.nextBatch():
            ...
            self.save_imputation(imputed,batchid,data_x_lengths,data_deltaPre,data_y,isTrain,filenames)
            batchid+=1
            impute_tune_time=1
    @property
    def model_dir(self):
    ...
    def save_imputation(self,impute_out,batchid,data_x_lengths,data_times,data_y,isTrain,filenames):
        if isTrain:
        ...
        # impute_out:ndarray
        for imp in range(0,len(impute_out)):
            oneSeries = impute_out[imp]
            resultFile.writelines("begin-%s\r\n"%filenames[imp])
        ...
        for imp in range(0,len(data_times)):
            oneSeries = data_times[imp]
            resultFile.writelines("begin-%s\r\n"%filenames[imp])
            for oneClass in oneSeries:
                for i in oneClass:
                    resultFile.writelines(str(i)+",")
        ...
        for imp in range(0,len(data_y)):
            oneSeries = data_y[imp]
            resultFile.writelines("begin-%s\r\n"%filenames[imp])
            for oneClass in oneSeries:
                resultFile.writelines(str(oneClass[1])+",")
            resultFile.writelines("\r\n")
            resultFile.writelines("end\r\n")
        resultFile.close()
