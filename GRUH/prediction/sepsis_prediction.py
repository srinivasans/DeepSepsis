...
from datautils import read_imputed
import GRUI
if __name__ == '__main__':
    ...
    parser.add_argument('--data-path', type=str, default="../imputation/imputation_train_results/WGAN/")
    ...
    parser.add_argument('--celltype', type=str, default='GRUI')
    #0 false 1 true
    ...
    for d in data_paths:
        ...
        dt_train=read_imputed.ReadImputedPhysionetData(args.data_path)
        dt_train.load()
        
        dt_test=read_imputed.ReadImputedPhysionetData(args.data_path.replace("imputation_train_results","imputation_test_results"))
        dt_test.load()
        ...
        for lr in lrs:
            ...
            with tf.Session(config=config) as sess:
                if args.celltype == 'GRUI':
                        model = GRUI.grui(sess,
                                args=args,
                                dataset=dt_train,
                                test_set = dt_test
                                )
