...
class lstm():
    ...
    def __init__(self,
        ...
        self.y_mask = tf.placeholder(tf.float32,
                                     [None, self.n_steps, self.n_classes])
        self.utp = tf.placeholder(tf.float32,
                                  [None, self.n_steps, self.n_classes])
        self.ufp = tf.placeholder(tf.float32,
                                  [None, self.n_steps, self.n_classes])
        self.ufn = tf.placeholder(tf.float32,
                                  [None, self.n_steps, self.n_classes])
    ...
    def build(self):
        ...
        positive_class = tf.reduce_sum(
            tf.cast((self.y > 0.5), dtype=tf.float32))
        padding = tf.reduce_sum(tf.cast((self.y_mask < 0.5), dtype=tf.float32))
        negative_class = tf.reduce_sum(
            tf.cast((self.y < 0.5), dtype=tf.float32)) - padding



        pos = tf.reduce_sum(
            tf.cast((self.train_data.y  > 0.5), dtype=tf.float32))

        pad = tf.reduce_sum(tf.cast((self.train_data.y_mask < 0.5), dtype=tf.float32))

        neg = tf.reduce_sum(
            tf.cast((self.train_data.y < 0.5), dtype=tf.float32))-pad

        self.class_ratio = neg/(pos)  # - Change class ratio - left for experimentation
        #
        # self.class_ratio = negative_class/negative_class

        self.utility = (self.y * self.output) * self.utp + (
                    self.y * (1.0 - self.output)) * self.ufn + (
                                   (1.0 - self.y) * self.output) * self.ufp
        ...
        self.normalized_utility = (self.utility - self.u_nopred) / (
                    self.u_optimal - self.u_nopred)
        ...
        self.act = tf.nn.weighted_cross_entropy_with_logits(targets=self.y,
                                                            logits=self.pred,
                                                            pos_weight=self.class_ratio)
        ...
        self.y_pred = tf.cast(((self.output * self.y_mask) > self.threshold),
                              dtype=tf.int32)
        ...
        self.metric_op = tf.group(self.acc_update, self.accuracy,
                                  self.tp_update, self.tp, self.fp_update,
                                  self.fp, self.tpr, self.tn_update, self.tn,
                                  self.fn_update, self.fn, self.fpr,  self.class_ratio)
        ...
        norm_utility_sum = tf.summary.scalar("Normalized Utility",
                                             self.normalized_utility)
        utility_sum = tf.summary.scalar("Utility", self.utility)
        unopred_sum = tf.summary.scalar("Utility - No prediction",
                                        self.u_nopred)
        uopt_sum = tf.summary.scalar("Utility Optimal", self.u_nopred)

        self.sum = tf.summary.merge(
            [loss_sum, acc_sum, tp_sum, tpr_sum, fpr_sum, fp_sum, utility_sum,
             norm_utility_sum, unopred_sum, uopt_sum])
    ...
    def train(self):
        ...
        while epochcount < self.epochs:
            ...
            dataset.shuffle()
            for train_x, train_y, train_m, train_delta, train_xlen, y_mask, utp, ufp, ufn, files, _ in dataset.getNextBatch():
                _, loss, summary_str, acc, _, cr, tp, fp, tn, fn = self.sess.run(
                    [self.train_op, self.loss, self.sum, self.accuracy,
                     self.metric_op, self.class_ratio, self.tp, self.fp, self.tn, self.fn], feed_dict={
                        self.x: train_x,
                        self.y: train_y,
                        self.m: train_m,
    ...
    def test(self, counter=None, val=True, checkpoint_dir=None, test_epoch=100, generate_files=False):
        ...
        tf.local_variables_initializer().run()
        for test_x, test_y, test_m, test_delta, test_xlen, y_mask, utp, ufp, ufn, files, _ in dataset.getNextBatch():
            _, summary_str, acc, pred, val_loss, tp, fp, tn, fn = self.sess.run(
                [self.metric_op, self.sum, self.accuracy, self.output, self.loss, self.tp, self.fp, self.tn, self.fn], feed_dict={
                    self.x: test_x,
                    self.y: test_y,
                    self.m: test_m,
            ...
            for i in range(0, test_xlen.shape[0]):
                ...
                targets_ind.append(list(test_y[i, 0:test_xlen[i]]))
